...
int main()
{
    ...
    // Debugging on 04.13.2016
    bool isSetUp = true;
    for (int i=0; i<words.size(); i++)
    {
        if (!b->is_word_in_tree(words[i]))
        {
            isSetUp = false;
        }
    }
    if(isSetUp)
        cout << "Set Up Correct!" << endl;
    ...
    /*
     cout << "PUTONTI'S RESULTS" << endl;
     for (int i = 0; i < 35; i++)
     {
     cout << is_in[i] << endl;
     }
     */  // what the tree search comes up with
    
    //The whole genome is put into genomeContents
    in.clear(); in.close();
    for (int i = 0; i < genomeContents.length(); i++)
    {
        if (genomeContents[i] == 'a' || genomeContents[i] == 'g') genomeContents[i] = 'r';  //purine
        else genomeContents[i] = 'y';   //pyrimidine
    }
    cout << genomeContents << endl;
    
    //using .find() searching for the reads in the genome
    for (int i = 0; i <reads.size(); i++)
    {
        if (genomeContents.find(reads[i]) != -1)
            cout << reads[i] << ": debugging string search: 1;\t\tPutonti's tree search: " << is_in[i] << endl;
        else cout << reads[i] << ": debugging string search: 0;\t\tPutonti's tree search: " << is_in[i] << endl;
    }
    //Results from string search should equal tree search
...
bool get_reads(char * file_name, vector<string> &r)
{
    ...
    {
        in.getline(word, 20, '\n');
        for (i = 0; i<10; i++) { if (word[i]<97) word[i] += 32; }
        //makes it lowercase
        for (i = 0; i<10; i++)
        {
            if (word[i] == 'a' || word[i] == 'g') word[i] = 'r';  //purine
